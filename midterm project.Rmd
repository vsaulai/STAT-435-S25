...
spurs_shots_2017_a <- spurs_shots_2017 %>%
  select(yearSeason, typeShot, locationX, locationY, distanceShot, isShotMade)
spurs_shots_2017_shotmade <- spurs_shots_2017_a %>%
  select(locationX, locationY, typeShot)
spurs_shots_2017_xy <- spurs_shots_2017_a %>%
  select(locationX, locationY)
typeshot_2 <- spurs_shots_2017_shotmade$typeShot
#2017 Shot Chart (scatter_plot)
shot_map_plot_17 <- ggplot(spurs_shots_2017_xy, aes(x = locationX, y = locationY, color = typeshot_2)) +
  annotation_custom(rasterGrob(court_raster, width = unit(1, "npc"), height = unit(1, "npc")), #background image import scaled
...
spurs_shots_2018_a <- spurs_shots_2018 %>%
  select(yearSeason, typeShot, locationX, locationY, distanceShot, isShotMade) #format df
spurs_shots_2018_shotmade <- spurs_shots_2018_a %>%
  select(locationX, locationY, typeShot)
spurs_shots_2018_xy <- spurs_shots_2018_a %>%
  select(locationX, locationY)
typeshot_3 <- spurs_shots_2018_shotmade$typeShot
#2018 Shot Chart (scatter_plot)
shot_map_plot_18 <- ggplot(spurs_shots_2018_xy, aes(x = locationX, y = locationY, color = typeshot_3)) +
  annotation_custom(rasterGrob(court_raster, width = unit(1, "npc"), height = unit(1, "npc")), #background image import scaled
                    xmin = -265, xmax = 255, ymin = -47, ymax = 460) +
  geom_point(alpha = 0.3, size = 1) +
  scale_color_manual(values = c("2PT Field Goal" = "blue", "3PT Field Goal" = "red")) +
